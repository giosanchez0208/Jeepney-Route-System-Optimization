"""
evaluate_runs.py -- evaluate the findings of all final optimizations (Chapter 4.4-4.5).

Run AFTER scripts/run_optimization.py has produced outputs under outputs/final_runs/<tag>/opt_<ts>/.
...
Figures -> outputs/figures/ ; a stats summary is printed.
"""
...
from jeepney.evaluation_metrics import jaccard_similarity, wasserstein_2d, shannon_entropy, graph_edit_distance
...
def _load_env():
    ...
    city = reuse_citygraph("data/cache/profile_p1.pkl")
    ddm = reuse_ddm("data/cache/ddm_8am.pkl")
...
def _resim(city, ddm, config, routes, fleet=None):
    ...
    from jeepney.route import route_from_coords
...
def resim_evaluation(runs: dict[str, Path], baseline_k: int = 7, max_workers=None):
    ...
    from jeepney.route import route_from_coords
    from jeepney.simulation_parallel import ParallelSimulationRunner
    ...
    from jeepney.pheromone import PheromoneMatrix
...
def _get_base_map():
    ...
    from jeepney.pipeline import reuse_citygraph
    cg = reuse_citygraph("data/cache/profile_p1.pkl")
...
def main():
    ...
    if not runs:
        print("No runs under outputs/final_runs/ yet. "
              "Launch one with: python -m scripts.run_optimization --tag p1 --seed 1")
        return
