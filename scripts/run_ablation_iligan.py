...
Usage (all arms share one --tag folder so they can be launched in separate terminals/machines):
    ...
    python -m scripts.run_ablation_iligan --tag run1 --arms ga_only
    python -m scripts.run_ablation_iligan --tag run1 --arms aco_only
...
from scripts.run_ablation_toy import ARMS, ARM_LABEL, ARM_COLOR, load_history
