...
It reads a run directory written by jeepney/optimizer_telemetry.py:
...
This is a TWO-STEP module (it renders a *finished* run, it does not run the optimizer itself):
    python -m scripts.run_toy_optimization    # step 1: runs the optimizer (~minutes), writes telemetry
    python -m scripts.figures.fig_optimization  # step 2: renders the NEWEST run under outputs/ (fast)
    python -m scripts.figures.fig_optimization --run-dir outputs/<...> --only fig_opt_convergence
...
from scripts.figures.fig_environment import set_pub_style, IMG_DIR
