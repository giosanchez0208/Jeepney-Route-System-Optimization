...
Everything reads from the pickles in data/cache/ -- no CityGraph/TravelGraph rebuild, no simulation --
so it is fast and safe to re-run.

    python -m scripts.figures.fig_environment                  # regenerate all
    python -m scripts.figures.fig_environment --only ddm_time_comparison
    python -m scripts.figures.fig_environment --list           # show figure names

Outputs land in outputs/figures/. Copy the ones you want to keep into docs/figures/.
"""
...
# jeepney.pipeline -> jeepney.city_graph pulls in pyrosm (a heavy OSM dependency present only on the
...
def load_assets():
    from jeepney.pipeline import reuse_citygraph, reuse_ddm  # lazy: pulls in pyrosm
