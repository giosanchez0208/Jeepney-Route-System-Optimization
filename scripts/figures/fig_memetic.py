...
Heavy deps (jeepney.pipeline -> pyrosm, the simulator) are imported lazily inside build_scene(), so
the pure-matplotlib renderers and their tiny test import and run anywhere.

    python -m scripts.figures.fig_memetic          # build the scene (3 sims), render all 4
...
from scripts.figures.fig_environment import (
...
def build_scene(num_routes=6, seed_a=1, seed_b=2, fleet=None, config_path=TOY_CONFIG):
    ...
    from jeepney.pipeline import generate_route_system, build_pheromone_matrix
    ...
    from jeepney.genetic import Chromosome, MemeticAlgorithm
