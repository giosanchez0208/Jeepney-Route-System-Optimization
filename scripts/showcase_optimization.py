...
Stitches the pieces we already have (toy simulation + its draw(), jeepney.pipeline operators,
...
Heavy deps (the simulator, pyrosm-backed jeepney modules) are imported lazily inside the functions, so this
...
fig_memetic so the showcase matches the thesis figures exactly.
    ...
    python -m scripts.showcase_optimization        # smoke render of all 7 beats
    python -m scripts.showcase_optimization --full # slide-quality (slow)
"""
...
from scripts.figures.fig_memetic import (
...
from scripts.figures.fig_environment import set_pub_style, _extent, BASE_SIZE
...
def make_config(smoke: bool = True, city: str = "toy", **overrides) -> dict:
    ...
    if city == "toy":
        ...
        cfg = dict(
            ...
            cg_pkl="data/cache/profile_p1.pkl",           # cached CityGraph (no OSM re-extraction)
            ddm_pkl="data/cache/ddm_8am.pkl",             # cached 08:00 demand sampler
...
def _build_sim(env: dict, routes: list, config: dict):
    ...
    import copy
    from jeepney.travel_graph import TravelGraph
    from jeepney.jeep import Jeep
    from jeepney.jeep_system import JeepSystem
    from jeepney.passenger_generator import PassengerGenerator
    from jeepney.simulation import Simulation
...
def _make_chrom(env: dict, routes: list, res):
    ...
    from jeepney.genetic import Chromosome
    from jeepney.pipeline import build_pheromone_matrix
...
def build_population(env: dict, config: dict) -> list:
    ...
    import random
    from jeepney.pipeline import generate_route_system
    members = []
...
def build_crossover_scene(env: dict, members: list, config: dict) -> dict:
    ...
    """
    from jeepney.genetic import MemeticAlgorithm
    from jeepney.local_search import ACOLocalSearch
    city = env["city"]
...
def apply_obvious_mutation(env: dict, routes: list, pheromones, config: dict, base_cost=None) -> dict:
    ...
    from jeepney.pipeline import mutate_attraction, mutate_repulsion, mutate_pruning
...
def run_short_optimization(config: dict) -> str:
    ...
    import subprocess
    import sys
    from scripts.figures.fig_optimization import find_latest_run
...
def render_convergence(run_dir: str, config: dict) -> dict:
    ...
    from scripts.figures.fig_optimization import load_run, fig_opt_convergence, fig_opt_evolution
...
def render_evolution_animation(run_dir: str, out: str, max_frames: int = 24, hold_last: int = 6,
    ...
    import io
    import scripts.figures.fig_optimization as fo
