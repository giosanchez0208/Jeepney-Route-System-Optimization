...
Once spatial passenger demand is calculated via the \ac{DDM}, the programmatic distribution of these generated passenger journeys is handled deterministically via the schedule generation routine of the passenger generator module. Rather than spawning a fixed cohort all at once, the generator produces a continuous stochastic arrival stream governed by a target rate \texttt{spawn\_rate\_per\_hour}. Arrivals are scheduled in repeating 100-tick windows: for each window an expected count $\lambda = (\texttt{rate\_per\_hour}/3600)\cdot(100\,\Delta t)$ is perturbed by a Gaussian draw of standard deviation \texttt{stdev}, and the resulting passengers are scattered uniformly across that window's 100 ticks. The window regenerates every 100 ticks, so demand arrives continuously across the entire simulation horizon rather than in a single burst, which also avoids a structural spawning bottleneck on the first tick. Manual testing across discrete passenger volume levels demonstrated high consistency and framework stability, justifying the omission of more complex time-varying or temporally inhomogeneous Poisson arrival processes while preserving the essential spatial heterogeneity of demand.
