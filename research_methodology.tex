...
Once spatial passenger demand is calculated via the \ac{DDM}, the passenger generation phase governs the programmatic distribution of these generated passenger journeys. Rather than spawning a fixed cohort all at once or assuming a uniform spatial layout, the generator produces a continuous stochastic arrival stream where arrivals at individual nodes are modeled as Poisson-distributed processes. Crucially, the model explicitly articulates the interdependence between these Poisson-distributed arrivals and sociodemographic variables. The arrival rate $\lambda_i$ at any given node $i$ is directly coupled to macro-scale demographic weights, network betweenness centrality, and empirical TomTom speed ratios derived from the \ac{DDM}. This positioning emphasizes the framework’s capacity to capture demand heterogeneity, ensuring that the stochastic generation process is grounded in empirical demographic weighting rather than a uniform distribution. By linking micro-scale stochastic passenger arrivals to macro-scale census and centrality data, this adjustment strengthens the model's behavioral realism. During execution, arrivals are scheduled across the simulation horizon in repeating 100-tick windows: for each window, the aggregate expected count $\lambda = (\texttt{rate\_per\_hour}/3600)\cdot(100\,\Delta t)$ is perturbed by a Gaussian draw of standard deviation \texttt{stdev}, and the resulting passengers are stochastically distributed across the road network using Walker's Alias Method based on the DDM probabilities $P_i$. This integrated spatial density framework ensures that demand is both temporally continuous and stochastically grounded in the city's demographic layout.
...
\subsection{Opportunistic Boarding under Weight Tolerance}
\noindent
The capacity rule governs \emph{whether} a vehicle can accept a passenger, but not \emph{which} vehicle a waiting passenger accepts. Real commuters do not rigidly wait for a single pre-planned jeepney; when an alternative jeepney arrives sooner and still carries them acceptably close to their destination, a rational passenger boards it rather than continue waiting \citep{iseki2009transfers}. The framework encodes this behaviour through a single tolerance parameter.

Each waiting passenger has a planned boarding route, taken from the least-cost journey computed on the multi-layer \texttt{TravelGraph}. When a jeepney reaches the passenger's stop, the boarding decision proceeds as follows. If the arriving vehicle belongs to the planned route, the passenger boards directly. Otherwise the passenger evaluates the alternative: let $c_{\text{alt}}$ be the generalized in-vehicle cost of riding the arriving (non-planned) vehicle from the current stop to the passenger's intended alighting node, and let $c_{\text{plan}}$ be the generalized cost of the planned ride. The passenger boards the alternative if and only if
\[
c_{\text{alt}} \;\le\; c_{\text{plan}} + \delta_{\text{tol}},
\]
where $\delta_{\text{tol}}$ (the \texttt{weight\_tolerance}, expressed in the same \ac{EIVM} units as the edge weights) is the maximum additional in-vehicle cost a passenger will accept in exchange for boarding immediately. Setting $\delta_{\text{tol}} = 0$ recovers strict adherence, in which a passenger only ever boards the planned route, while larger values permit progressively more opportunistic transfers. A non-planned vehicle is a viable alternative only if its route actually reaches the passenger's alighting node, so the mechanism never strands a passenger on a route that cannot serve their destination.

To quantify the effect of this behaviour on the individual commuter, each completed passenger's realized travel time is compared against a friction-free baseline. The \emph{expected} travel time $T^{\text{exp}}$ is the time the journey would take in the absence of any waiting or transfer delay---purely the walking and riding legs traversed at their respective speeds:
\[
T^{\text{exp}} = \frac{d_{\text{walk}}}{v_{\text{walk}}} + \frac{d_{\text{ride}}}{v_{\text{ride}}},
\]
where $d_{\text{walk}}$ and $d_{\text{ride}}$ are the total walking and in-vehicle distances of the passenger's journey and $v_{\text{walk}}, v_{\text{ride}}$ are the pedestrian and jeepney speeds. The \emph{actual} travel time $T^{\text{act}}$ is measured directly from the simulation as the elapsed time between the passenger's spawn and arrival, and therefore embeds all realized waiting, boarding, and transfer friction. The realized delay is their difference, $\Delta T = T^{\text{act}} - T^{\text{exp}}$.

The behavioural hypothesis---that permitting opportunistic transfers lowers an individual's realized delay---is tested by partitioning completed passengers into those who boarded their planned route (Group~A) and those who boarded an acceptable alternative (Group~B), and comparing their delay distributions with a one-sided Mann--Whitney $U$ test across a sweep of tolerance values. The non-parametric test is appropriate because the delay distributions are heavy-tailed rather than Gaussian.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{chap3/figures/fig_simulation_loop.png}
    \caption{Agent-based simulation (illustrative schematic). \emph{Left:} the deterministic per-tick event loop, ordered so that vehicles advance before passenger interaction is resolved; the run terminates at the fixed horizon and emits the fitness $F_{\text{sim}}$. \emph{Right:} the passenger finite-state machine over its four valid states, showing the transitions that carry an agent between walking, waiting, riding, and the terminal done state.}
    \label{fig:simulation_loop}
\end{figure}

This ordering, summarized in Figure~\ref{fig:simulation_loop}, matters because vehicle movement must be updated before passenger interaction. A passenger boards only after the jeepney has reached the relevant node for that tick. The simulation does not allow direct interaction between passengers and vehicles. Instead, jeepneys emit events, and the main controller handles boarding, capacity checking, and alighting through the TravelGraph. This event-driven structure keeps the model modular and computationally efficient.
...
\subsection{Gap-Gated Lamarckian Acceptance}
\noindent While the simulation fitness provides a high-fidelity evaluation, executing the temporal physics engine requires updating thousands of independent agent states across hundreds of simulation ticks. During the Lamarckian local search phase (detailed in Section 3.6.2), the algorithm evaluates multiple candidate route perturbations per chromosome, and running the full simulation for each minor structural mutation would severely bottleneck the evolutionary search. To resolve this, mutation acceptance is gated not by a full re-simulation but by a cheap, internally consistent proxy: the Proportional Demand-Service Disparity already maintained within the pheromone matrix (Section~\ref{sec:demand_service_gap}). Substituting a lightweight approximate criterion for the expensive objective during intra-generational local search is a standard device in surrogate-assisted and memetic optimization \citep{jin2005survey}.

Because the local search operators are themselves steered by this disparity---attracting service toward underserved high-pheromone corridors while repelling service from oversupplied segments and pruning tortuous detours---using the same quantity as the acceptance criterion unifies the direction and the acceptance of every move under a single pheromone-derived signal. Concretely, the total disparity of a route system $\mathbf{R}$ is taken as the $L_1$ distance between the normalized demand and supply distributions,
\[
D(\mathbf{R}) = \sum_{(i,j)\in E} \left| P_{ij} - S_{ij} \right|,
\]
where $P_{ij}$ and $S_{ij}$ are the edge-level demand and supply shares defined in Section~\ref{sec:demand_service_gap}. A mutation is provisionally kept only if it strictly reduces $D(\mathbf{R})$, and is otherwise reverted. This favors consolidated, coherent corridors over fragmented configurations, thereby mitigating the transfer penalties empirically known to deter ridership \citep{iseki2009transfers}, without ever invoking the temporal engine. Once the local search phase concludes, the fully mutated chromosome is routed back through the microscopic temporal simulation to receive its authoritative $F_{\text{sim}}$ fitness score before the \ac{GA} advances to the next generation, so that selection, elitism, and replacement remain governed exclusively by the high-fidelity objective.
...
\begin{enumerate}
    ...
    \item \textbf{Selection and Elitism:} To guarantee monotonicity of the best solution, elitism is applied where the single best chromosome ($N_{\text{elite}} = 1$ in the Iligan network configuration, and $N_{\text{elite}} = 2$ in synthetic toy network configurations) is copied directly into the next generation. These elite chromosomes are preserved completely unchanged, bypassing all subsequent crossover, mutation, and local search operations. For the remaining population slots, parents are selected using tournament selection with tournament size $k_{\text{tournament}} = 3$. This selection mechanism samples three individuals uniformly at random from the population and selects the top two (lowest $F_{\text{sim}}$ cost) to serve as parents for recombination.
    \item \textbf{Crossover and Inheritance:} Topological Hub Crossover produces the child's route geometry by recombining the high-demand trunk from the fitter parent with non-overlapping feeder routes from the other. Simultaneously, through Epigenetic Inheritance, the child instantly receives a fitness-weighted blend of the parents' pheromone maps ($\tau^{\text{child}}_{ij} = w_A \tau^A_{ij} + w_B \tau^B_{ij}$ where $w_A = C_B / (C_A + C_B)$). This warm-start mechanism bypasses the need to run an expensive pre-simulation on the child just to discover where the topological weaknesses lie.
    \item \textbf{Lamarckian Local Search and Intensity Windowing:} The mutation gate decides whether local search should intervene, utilizing the current mutation rate as the trigger probability. Guided by the inherited pheromone map, the mutator attempts structural tweaks via three operators: spatial attraction, redundancy repulsion, and tortuosity pruning. Each tweak is accepted only if it strictly reduces the Proportional Demand-Service Disparity $D(\mathbf{R})$ already maintained in the pheromone matrix (Section~\ref{sec:demand_service_gap})—the same stigmergic gap signal that steers the operators—so acceptance never invokes the temporal engine. Otherwise, the original geometry is restored. The local search intensity parameter ($I_{\text{local}}(g)$) controls the search radius by scaling the edge window size ($W$) for the repulsion and pruning operators. Specifically, for redundancy repulsion, the search window size is $W = \max(2, \min(\lfloor 15 \cdot I_{\text{local}}(g) \cdot 0.5 \rfloor, n / 3))$ edges, where $n$ is the number of edges in the target route; for tortuosity pruning, the maximum bypass segment length is $W = \max(3, \min(\lfloor 15 \cdot I_{\text{local}}(g) \rfloor, 20))$ edges. As $I_{\text{local}}(g)$ decays linearly from $1.0$ to $0.1$ across generations, the search window automatically tightens from broad structural shifts to highly surgical local route refinements.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.92\textwidth]{chap3/figures/fig_optimization_pipeline.png}
    \caption{The memetic \ac{GA}--\ac{ACO} generational loop (illustrative schematic). Each generation evaluates its parents (one agent simulation per chromosome, yielding a pheromone map), applies elitism and tournament selection, recombines through Topological Hub Crossover with epigenetic pheromone inheritance, refines each offspring with gap-gated Lamarckian local search, and evaluates the child exactly once before testing the dual convergence criteria. The adaptive controller scales the mutation rate and local-search intensity with the stagnation counter, resetting them when a new best solution is found.}
    \label{fig:optimization_pipeline}
\end{figure}

This sequencing, illustrated in Figure~\ref{fig:optimization_pipeline}, is computationally critical. By gating Lamarckian mutations on the lightweight demand-service disparity and reserving the temporal simulation strictly for post-mutation evaluation, the framework limits the most expensive operation—the agent-based physics engine—to exactly one pass per child. If local search required full simulations to test every minor structural tweak, or if epigenetic inheritance did not exist to map demand proactively, the evolutionary algorithm would bottleneck immediately. The pipeline therefore preserves optimization correctness, temporal efficiency, and scientific traceability.
...
% ────────────────────────────────────────────────────────────
\subsection{Parallel Execution Mechanics and Memory Isolation}
\label{sec:parallelization}
\noindent
Evaluating candidate transit networks requires running a complete agent-based passenger simulation for each chromosome. Because these simulations are highly CPU-bound and contain thousands of interacting agents, executing them sequentially creates a major computational bottleneck. To leverage multi-core processors, this study implements a parallel simulation execution system utilizing process-level isolation rather than multi-threading to bypass Python's Global Interpreter Lock (GIL).

To make parallelization efficient and scalable, the following mechanics are implemented:
\begin{enumerate}
    \item \textbf{One-Time Worker Initialization:} Heavy static objects—namely the \texttt{CityGraph} and the \texttt{DirectDemandSampler}—are instantiated exactly once when a worker process spawns, caching them in process-local global variables. This eliminates the need to serialize and transfer these heavy network objects across the Inter-Process Communication (IPC) boundary for each simulation evaluation, reducing the per-evaluation transmission cost to $O(1)$.
    \item \textbf{Lightweight Route Serialization:} When a \texttt{Route} object is transmitted to a worker process, custom serialization hooks strip it of its complex node and edge references, sending only its ordered sequence of coordinate hashes (\texttt{path\_keys}). The worker reconstructs the route in its local memory using a fast lookup table mapped to the cached \texttt{CityGraph}.
    \item \textbf{TravelGraph Caching:} Reconstructing the multi-layered \texttt{TravelGraph} for each candidate network is computationally expensive. If a worker process receives a route set identical to its previous run (detected via a hash of route IDs), it reuses the cached \texttt{TravelGraph} rather than rebuilding it.
    \item \textbf{Persistent Process Pools:} To avoid the startup overhead of spawning worker processes and loading GIS files in every generation, workers are managed within a persistent process pool that remains active throughout the entire optimization run.
    \item \textbf{Memory Isolation and Reclaim:} Large-scale agent simulations with cyclic references are prone to memory leaks. After running a simulation, the worker process manually drops the passenger and vehicle agents and invokes explicit garbage collection (\texttt{gc.collect()}) to reclaim memory before executing the next task.
\end{enumerate}
...
% ────────────────────────────────────────────────────────────
% [Removed] "Surrogate Fidelity Validation (Spearman and Kendall Rank Correlation)" subsection.
% The static surrogate evaluator was deprecated in favor of gap-gated Lamarckian acceptance
% (Section~\ref{sec:demand_service_gap}); a surrogate-vs-simulation rank-fidelity validation is
% therefore no longer applicable. The remaining post-optimization metrics (Jaccard / Frechet / GED,
% Wasserstein, Shannon entropy) are unaffected by the removal.