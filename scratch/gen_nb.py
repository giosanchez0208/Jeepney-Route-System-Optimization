"""Generate rnd_1_ticks_and_rate.ipynb — optimized for ~4hr on 8-core machine."""
import json
...
def main():
    ...
    cells.append({"cell_type": "code", "execution_count": None, "metadata": {}, "outputs": [], "source": [
        ...
        "else:\n",
        "    print(\"\\n--- PRODUCTION MODE ---\")\n",
        "    ROUTE_COUNTS     = [3, 6, 9, 12, 15, 18]\n",
        "    PASSENGER_RATES  = [200, 600, 1000]\n",
        "    REPLICATIONS     = 5\n",
        "    JEEPS_PER_ROUTE  = 10\n",
        "    SPT_VALUES       = [10, 20, 40, 80, 160, 320]\n",
        "    FIXED_SIM_S      = 1800     # 30 min simulated for SPT sweep\n",
        "    BISECT_BOUNDS    = (100, 2000) # ticks\n",
        "    BISECT_ITERS     = 5\n",
        "\n",
        "CV_THRESHOLD = 0.05\n",
    ...
    cells.append({"cell_type": "code", "execution_count": None, "metadata": {}, "outputs": [], "source": [
        ...
        "    \"\"\"\n",
        "    Runs REPLICATIONS simulations in parallel using ProcessPoolExecutor.\n",
        "    Workers cache TravelGraph so it's only built once per route set.\n",
        "    Returns (cv, mean_score, scores_list).\n",
        "    \"\"\"\n",
    ...
    cells.append({"cell_type": "code", "execution_count": None, "metadata": {}, "outputs": [], "source": [
        ...
        "            mid = (lo + hi) // 2\n",
        "            t0 = time.time()\n",
        "            cv, mean_fit, scores = evaluate_cv_parallel(routes, optimal_spt, mid, pass_rate)\n",
        "            wall = time.time() - t0\n",
        "            status = 'PASS' if cv <= CV_THRESHOLD else 'FAIL'\n",
    ...
    nb = {
        ...
        "metadata": {"kernelspec": {"display_name": ".venv", "language": "python", "name": "python3"},
                      "language_info": {"codemirror_mode": {"name": "ipython", "version": 3},
                                        ...
                                        "pygments_lexer": "ipython3", "version": "3.11.3"}},
        "nbformat": 4, "nbformat_minor": 5
    }
    with open("rnd_1_ticks_and_rate.ipynb", "w", encoding="utf-8") as f:
