"""Smoke test for notebooks/showcase_iligan.ipynb (Iligan-specific paths).

The shared mechanics are covered by tests/test_showcase_toy.py; this exercises the parts
...
test (it loads ~100 MB of pickles and runs a couple of real-city sims) but still a short sanity check.
    ...
    python tests/test_showcase_iligan.py
"""
...
import scripts.showcase_optimization as S
from scripts.figures.fig_memetic import fig_hub_crossover


def main():
    if not os.path.exists("data/cache/profile_p1.pkl"):
        print("SKIP  cached Iligan CityGraph pickle not present (data/cache/profile_p1.pkl)")
        return
